...
function parseAccountJs(exportPath) {
  // Try to find and parse account.js to get username
  const accountPaths = [path.join(exportPath, "data", "account.js"), path.join(exportPath, "account.js")];

  for (const accountFile of accountPaths) {
    if (fs.existsSync(accountFile)) {
      try {
        ...
        const parts = Object.values(context.window.YTD.account);
        for (const part of parts) {
          for (const item of part) {
            if (item.account && item.account.username) {
...
<head>
    ...
    <title>X/Twitter Archive${username ? ` for @${username}` : ""}</title>
    <style>
        :root {
            --bg-color: #15202b;
            --card-bg: #192734;
        ...
        <header>
            <div class="sticky-top">
                <div class="header-row">
                    <h1>X/Twitter Archive${username ? ` for @${username}` : ""}</h1>
                    <a href="https://github.com/sergeyk/x-backup" target="_blank" rel="noopener" class="github-link">
    ...
    <script>
        ...
        function filterAndSort() {
            ...
            tweets.forEach((tweet) => {
                ...
                if (typeMatch && likesMatch && searchMatch) {
...
Example:
  ...
  const html = generateHtml(tweets, mediaDir || "", username);
